...
CATEGORY_HASHTAGS = {
    ...
    "Default": ["#AmazonFinds", "#TrendingGadgets", "#SmartCartIndia", "#ViralProducts", "#DailyDeals"]
}
...
def sync_landing_page_to_github():
    ...
    try:
        ...
        subprocess.run([git_cmd, "add", "assets/*"], check=False, capture_output=True, shell=True, text=True, cwd=project_dir)
...
def record_upload(asin, name):
    ...
    new_data.to_csv(HISTORY_FILE, mode='a', index=False, header=not file_exists)
...
def start_daily_routine():
    ...
    try:
        ...
        for i, item in enumerate(products_found):
            ...
            voice_script = f"Check out the all new {safe_name}. Featuring {short_specs_summary}. Direct link to get this deal is pinned below!"
            print(f"💬 Generated script text: {voice_script}")
            # ───────────────────────────────────────────────────────────

            # Pass the constructed script text down into your newly updated editor module
            editor.create_pro_video(final_images, safe_name, video_path, voice_text=voice_script)
...
def run_manual_post(url):
    ...
    try:
        ...
        # Pass the constructed script text down into your newly updated editor module
        editor.create_pro_video(product['images'], safe_name, video_path, voice_text=voice_script)
